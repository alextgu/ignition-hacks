 ...
 */
export function createWorldLabsAdapter(env: NodeJS.ProcessEnv = process.env): WorldLabsAdapter {
...
/** Returns a process-wide singleton adapter, selected from `process.env` on first use. */
...
export function describeWorldLabsConfig(env: NodeJS.ProcessEnv = process.env): Record<string, unknown> {
